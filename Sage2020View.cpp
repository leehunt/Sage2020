...
CFont* CSage2020View::GetCustomFont(int* pFontGenerationIndex) {
  if (!(HKEY)m_accesibilityScaleKey) {
    m_accesibilityScaleKey.Open(
        HKEY_CURRENT_USER, _T("Software\\Microsoft\\Accessibility"), KEY_READ);
  }
  if ((HKEY)m_accesibilityScaleKey) {
    if (!m_accessibityRegKeyModificationEvent) {
      m_accessibityRegKeyModificationEvent.reset(
          ::CreateEvent(nullptr, TRUE, FALSE, nullptr));
      if (m_accessibityRegKeyModificationEvent) {
        ::RegNotifyChangeKeyValue(m_accesibilityScaleKey, FALSE,
                                  REG_NOTIFY_CHANGE_LAST_SET,
                                  m_accessibityRegKeyModificationEvent, TRUE);
      }
    }
  }
  if (m_accessibityRegKeyModificationEvent) {
    if (::WaitForSingleObject(m_accessibityRegKeyModificationEvent, 0) ==
        WAIT_OBJECT_0) {
      m_font.DeleteObject();
      m_sizChar.cx = 0;
      ::RegNotifyChangeKeyValue(m_accesibilityScaleKey, FALSE,
  ...
  if (pdc != NULL) {
    ...
    if (pfnt != NULL) {
      ...
      DWORD scale = 100;
      if (m_accesibilityScaleKey.QueryDWORDValue(_T("TextScaleFactor"),
                                                 scale) == ERROR_SUCCESS) {
